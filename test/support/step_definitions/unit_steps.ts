...
import { uniqueSort } from "../../../node_modules/cypress/types/jquery/index";
...
cy.get('[id="0"]').click()
cy.get('[data-cy=menuSubMenu_crop-settings] > .MuiCollapse-container').should('be.visible')
...
{
    cy.get('[data-cy="menuSubMenu_crop-settings"]').click()
    cy.url().should('include','admin/crop-settings/units')
})
// Scenario: Add unit

And ('I clicked button {string}',(button1)=>{
  
    cy.get('[data-cy="menuSubMenuAction_admin-units"]',{timeout:10000}).click()
 cy.get('[data-cy="addUnitButton"]',{timeout:10000}).contains(button1).click()
})
 ...
 datatable.hashes().forEach((ele: any) => {
    ...
    for (let propName in ele) {
        switch (propName) {
            ...
            case 'Tags':
                utilityHelper.setSingleDropdownValue('[data-cy="unitFormSelectTags"]',ele[propName])
                break;

            case 'Country (Optional)':
             
                utilityHelper.setSingleDropdownValue('[data-cy=unitFormSelectCountries]',ele[propName])
...
And ('I clicked {string} a modal appeared with following data:',(button)=>
{
    ...
    datatable.hashes().forEach((ele: any) => {
        ...
        for (let propName in ele) {
            switch (propName) {
                ...
                case 'Tags':
                    utilityHelper.setMultiDropdownValue('[data-cy="unitFormSelectTags"]',ele[propName],true)
                    break;
    
                    case 'Country (Optional)':
                        utilityHelper.setMultiDropdownValue('[data-cy="unitFormSelectCountries"]', ele[propName],true)
...
{
    cy.get('[data-cy="unitEditFormButtonClose"]').click()
})
...
When(' I clicked on three dots in end of row for unit Test a context menu with options {string} and {string} shows',(button2,button1)=>
{
